...
public class PaintServiceImpl implements PaintService {
    ...
    public Paint rebuildWithAdjustedAmount(Long id,int amount)//IDを元に追加
    {
        ...
        Paint nPaint =   withAmount(paint.getName(),paint.getType(),paint.getCategory(),paint.getRed(),paint.getGreen(),paint.getBlue(),paint.getAmount()+ amount,paint.getOownUser());
    ...
    public Paint add(PaintDto dto)
    {
        ...
        User user =userRepository.findByUserName(authentication.getName()).orElseThrow(() -> new RuntimeException("ユーザーが見つかりません"));
        Optional<Paint> existing = paintRepository.findByNameAndTypeAndRedAndGreenAndBlue(
       ...
       Paint paint = new Paint(dto.getName(),dto.getType(),dto.getCategory(),dto.getRed(),dto.getGreen(),dto.getBlue(),dto.getAmount(),user);
    ...
    public void decreaseByName(String name, int amount) {
       ...
       //for (Paint paint : paints) {
        ...
        List<Paint> paints = paintRepository.findByNameAndOownUser(name, user);
    ...
    private Paint withAmount(String name, String type, String category, int red, int green, int blue, int amount, User user) {
    ...
    public void mergeDuplicates()
    {
        ...
        for(Paint paint:allPaints) {
            String key =  paint.getOownUser().getuserName() + "|" +paint.getName() + "|" + paint.getType() + "|" + paint.getRed() + "|" + paint.getGreen() + "|" + paint.getBlue();
